 ...
 * 물리 상수
 * ═════════════════════════════════════════════════════════════════════════ */
#define WHEEL_DIAMETER_M        0.12f     /**< 바퀴 직경 [m] (줄자 실측)      */
 ...
 *
 * ⚠ 미확정 값입니다.
 *   라벨: GP36E13CPR, 1:27  ->  13 x 4체배 x 27 = 1404 (아래 값)
 *   그런데 ROS 측 실측은 7049 ticks/m 이고, 지름 0.12 m 기준 역산하면
 *   7049 x pi x 0.12 = **2657** 입니다. 1404 의 1.89배.
 *
 *   슬립은 ticks/m 을 '높이기만' 하므로 참값은 2657 이하입니다.
 *   1404 라면 +89% 슬립이 필요한데 그건 물리적으로 불가능합니다.
 *   -> 1404 는 확실히 틀렸고, 참값은 2657 근방입니다.
 *
 *   ★ encoder_probe 로 실측한 뒤 이 값을 바꾸십시오.
 *   ★ 바꾸면 측정 RPM 이 1.89배 작아져 PID 오차가 1.89배 커집니다.
 *     -> MOTOR_KP/KI 를 **먼저 절반으로 낮추고** 올려가며 재튜닝하십시오.
 *   ※ 이 상수는 펌웨어의 RPM 계산에만 쓰입니다. CAN 으로 나가는 원시 틱은
 *     영향을 받지 않으므로 ROS 측 encoder_scale 은 건드리지 않아도 됩니다.
 */
#define ENCODER_CPR             1404U
 ...
 *
 * 튜닝 단계별 권장값 (피드포워드 Zone2 = 2000 + 67.7*(RPM-31.8) 기준):
 *   4000 -> 약  61 RPM (≈0.38 m/s)   ← 1차 튜닝. 여기서 시작하십시오
 *   6000 -> 약  91 RPM (≈0.57 m/s)   ← 2차 검증
 *   9999 -> 146 RPM (≈0.92 m/s)      ← 최종 (= 제한 해제)
 */
 ...
 *
 *   ※ 이 곡선은 PWM 축 기준으로 실측된 값이라 ENCODER_CPR 을 바꿔도
 *     곡선 자체는 유효합니다. 다만 '목표 RPM' 의 물리적 의미가 바뀌므로
 *     CPR 수정 후 SMOOTH_RPM 이 정말 0.20 m/s 에 해당하는지 재확인하십시오.
 * ═════════════════════════════════════════════════════════════════════════ */
 ...
 * PID 이득
 *
 * ⚠ 현재 KP=2.0 은 피드포워드 Zone2 기울기(67.7 PWM/RPM) 대비 권한이 약 1/34 로
 *   사실상 개루프입니다. 그래서 RL 부상으로 좌측 추진이 1바퀴가 되어도
 *   좌측 PID 가 그걸 메우지 못하고 차체가 휩니다.
 *
 *   [재튜닝 순서]
 *     1) ENCODER_CPR 을 실측값으로 고친다  -> 실효 이득이 1.89배 커짐
 *     2) 그 상태에서 KP 를 **1.0 으로 낮춰** 시작
 *     3) 5 -> 10 -> 20 순으로 올리며 진동 확인
 *   KD 는 엔코더 양자화 잡음을 증폭하므로 0 에서 시작하는 것을 권합니다.
 * ═════════════════════════════════════════════════════════════════════════ */
#define MOTOR_KP                2.0f
#define MOTOR_KI                1.0f
